...
export const CommentsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchComments = useCallback(async (taskId: number) => {
    ...
    try {
      const res = await fetch(`https://teamup-five.vercel.app/api/comments/${teamInfo.team_id}/${taskId}`, {
        credentials: "include",
      });
      if (!res.ok) {
  ...
  const addComment = useCallback(
    async (comment: Omit<Comment, "comment_id" | "created_at" >) => {
      ...
      try {
        const res = await fetch(`https://teamup-five.vercel.app/api/comments/${teamInfo.team_id}`, {
          method: "POST",
  ...
  const deleteComment = useCallback(async (comment_id: number) => {
    ...
    try {
      const res = await fetch(`https://teamup-five.vercel.app/api/comments/${teamInfo.team_id}/${comment_id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) {
