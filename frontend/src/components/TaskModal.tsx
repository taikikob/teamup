...
function TaskModal({node, onClose, initialTaskId, initialPlayerId}: {node: Node; onClose: () => void; initialTaskId: string | null; initialPlayerId: string | null;}) {
    ...
    const fetchTasks = async () => {
        ...
        try {
            ...
            const res = await fetch(`https://teamup-server-beryl.vercel.app/api/teams/${teamInfo.team_id}/${node.id}/tasks`, {
                method: 'GET',
                credentials: 'include'
            });
    ...
    const saveTaskOrder = async (orderedTasks: any[]) => {
        ...
        try {
            const res = await fetch(`https://teamup-server-beryl.vercel.app/api/teams/${teamInfo.team_id}/${node.id}/tasks/order`, {
                method: 'PUT',
                headers: {
    ...
    const handleDeleteTask = async (task_id: number) => {
        ...
        try {
            const res = await fetch(`https://teamup-server-beryl.vercel.app/api/teams/${teamInfo.team_id}/${node.id}/tasks/${task_id}`, {
                method: 'DELETE',
                credentials: 'include'
            });
            if (res.ok) {
                // Refetch tasks
                const refreshed = await fetch(`https://teamup-server-beryl.vercel.app/api/teams/${teamInfo.team_id}/${node.id}/tasks`, {
                    method: 'GET',
                    credentials: 'include'
                });
