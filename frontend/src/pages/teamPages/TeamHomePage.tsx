...
function TeamHomePage() {
    ...
    const generateNewAccessCodes = async () => {
        ...
        try {
            const res = await fetch(`https://teamup-server-beryl.vercel.app/api/teams/${teamInfo.team_id}/newAccessCode`, {
                method: 'POST',
                credentials: 'include'
            });
    ...
    const disableAccessCodes = async () => {
        setLoadingButton(true);
        try {
            const res = await fetch(`https://teamup-server-beryl.vercel.app/api/teams/${teamInfo.team_id}/delAccessCode`, {
                method: 'DELETE',
                credentials: 'include'
            });
            if (res.status === 204) {
