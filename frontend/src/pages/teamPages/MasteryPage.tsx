...
function MasteryPage() {
    ...
    const saveFlowToBackend = useCallback(async (currentNodes: Node[], currentEdges: Edge[]) => {
        ...
        try {
            const response = await fetch(`/api/teams/${teamInfo?.team_id}/flow`, {
                method: 'POST',
                headers: {
    ...
    const updateNodeLabel = async (nodeId: string, newLabel: string) => {
        ...
        try {
            const res = await fetch(`/api/teams/${teamInfo.team_id}/node-label`, {
                method: 'POST',
        ...
        const fetchFlowData = async () => {
            try {
                ...
                if (teamInfo.is_user_coach) {
                    const res = await fetch(`/api/teams/${teamInfo.team_id}/flow?role=coach`, {
                        method: 'GET',
                        credentials: 'include'
                    });
                    if (res.status === 200) {
                    ...
                    const res = await fetch(`/api/teams/${teamInfo.team_id}/flow?role=player`, {
                        method: 'GET',
                        credentials: 'include'
                    });
                    if (res.status === 200) {
